...
The server starts at **http://localhost:8074**.

---

### Step 7 — Open the dashboard

Visit **http://localhost:8074/dashboard** in your browser.
...
```bash
curl http://localhost:8074/geo/8.8.8.8 \
...
```bash
curl -X POST http://localhost:8074/geo/batch \
  -H "X-API-Key: gip_your_token_here" \
...
```bash
curl http://localhost:8074/geo/me \
  -H "X-API-Key: gip_your_token_here"
```
